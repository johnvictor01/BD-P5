<template>
    <div>
      ...
      <div class="obras-container">
        <div v-for="obra in obrasFiltradas" :key="obra.id" class="obra-card">
          ...
          <p><strong>Tipo de Arte:</strong> {{ obra.tipoArte }}</p>
          <p><strong>Valor:</strong> R$ {{ obra.valor }}</p>
  ...
  export default {
    name: 'Filters',
    emits: ['adicionar-ao-carrinho'], // Emite um evento quando uma obra é adicionada ao carrinho
    setup(props, { emit }) {
      ...
      const obrasFiltradas = computed(() => {
        return obras.value.filter((obra) => {
          const tipoArteMatch =
            !filtros.value.tipoArte || obra.tipoArte === filtros.value.tipoArte;
          const valorMatch = obra.valor <= filtros.value.valor;
          const nomeMatch =
            !filtros.value.nome ||
            obra.nome.toLowerCase().includes(filtros.value.nome.toLowerCase());
          const alturaMatch =
